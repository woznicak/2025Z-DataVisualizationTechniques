...
for (el in vec) { if (proton(action = "login", login="johnins", password=el)=="Success! User is logged in!") {print(el)}}
employees[employees$surname=="Pietraszko",]
logs[logs$login=="slap",]
adr=logs[logs$login=="slap",c("host")]
which.max(table(adr))
proton(action = "server", host="194.29.178.16")
his=bash_history
for (el in his) { if (proton(action = "login", login="slap", password=el)=="Success! User is logged in!") {print(el)}}
"DHbb7QXppuHnaXGN"
